...
CC1101Driver *CC1101Driver::driverObject;
...
void CC1101Driver::Init(void)
{
   ...
   //  attachInterrupt(digitalPinToInterrupt(GDO0_PIN), std::bind(&CC1101Driver::TestIsr, this), RISING);

}



void IRAM_ATTR CC1101Driver::TestIsr()
{

    //CONSOLE.print("1");
    driverObject->IsrProcessing();
 ...
 */
void CC1101Driver::IsrProcessing()
{
    val_ISR =1;
}
...
void CC1101Driver::SpiWriteBurstReg(uint8_t addr, uint8_t const *buffer, uint8_t nbBytes, uint32_t guardTime_us)
{
    ...
    SPI.transferBytes(buffer, nullptr, nbBytes);
...
void CC1101Driver::SpiReadBurstReg(uint8_t addr, uint8_t *buffer, uint8_t nbBytes)
{
    ...
    SPI.transferBytes(nullptr, buffer, nbBytes);
...
uint8_t CC1101Driver::IsConnectedbyte(void)
{
    uint8_t bio = SpiReadStatus(0x31);
    return bio;
}
